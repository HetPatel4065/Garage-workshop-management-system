    ...
    try {
      ...
      if (settings?.notifications?.lowStock) {
        const owner = await Owner.findById(ownerId).select(
          "name email mobileNumber garageName",
        );
        ...
        if (owner?.email) {
          ...
          console.log(
            `[NOTIFY] Low stock email sent to ${owner.email} for ${item.name}`,
          );
        }
      }
    } catch (notifErr) {
...
export const createLowStockNotification = async (ownerId, item) => {
  ...
  console.log(
    `[DEBUG] Checking stock for ${item.name}: ${item.stock} / ${item.minLimit ?? 5}`,
  );
  if (item.stock <= (item.minLimit ?? 5)) {
    console.log(
      `[DEBUG] Item ${item.name} is low stock. Creating notification...`,
    );
    try {
      ...
      console.log(
        `[NOTIFY] In-app low stock notification created for ${item.name}`,
      );
    } catch (notifErr) {
      console.error(
        "[NOTIFY] In-app low-stock notification error:",
        notifErr.message,
      );
    }
  }
