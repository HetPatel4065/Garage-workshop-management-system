    ...
    <div className="flex items-center gap-3.5 py-3.5 border-b border-slate-100 dark:border-zinc-800/60 last:border-0">
      ...
      {copyable && (
        <button
          ...
          className="shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 dark:text-zinc-500 hover:bg-slate-100 dark:hover:bg-zinc-800 hover:text-slate-700 dark:hover:text-zinc-200 border border-slate-100 hover:border-slate-300 transition-colors"
        >
          {copied ? (
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Top Bar */}
        <div
          className="absolute top-0 inset-x-0 z-20 flex flex-wrap items-center justify-between gap-3 px-3 sm:px-4 md:px-6 py-3 bg-linear-to-b from-black/70 to-transparent"
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-white/90 bg-white/10 px-2 py-1 rounded-lg text-[10px] sm:text-xs font-black uppercase flex items-center gap-1">
              <ZoomIn className="w-3 h-3" /> {Math.round(zoom * 100)}%
            </span>
          </div>
        ...
        <div
          ref={containerRef}
          className="flex-1 flex items-center justify-center overflow-hidden relative w-full h-full px-2 sm:px-4"
          onClick={(e) => e.stopPropagation()}
          onWheel={handleWheel}
        >
          <AnimatePresence mode="wait">
            <motion.img
              ...
              className="max-w-full max-h-[80vh] sm:max-h-[85vh] md:max-h-[90vh] object-contain select-none"
        ...
        {photos.length > 1 && (
          <>
            <button
              ...
              className="absolute left-2 sm:left-4 md:left-6 top-1/2 -translate-y-1/2 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white flex items-center justify-center transition-all"
            >
            ...
            <button
              ...
              className="absolute right-2 sm:right-4 md:right-6 top-1/2 -translate-y-1/2 w-9 h-9 sm:w-11 sm:h-11 rounded-full bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white flex items-center justify-center transition-all"
            >
        ...
        {photos.length > 1 && (
          <div
            className="absolute bottom-0 inset-x-0 flex justify-center flex-wrap gap-2 px-4 py-4 bg-linear-to-t from-black/70 to-transparent"
            onClick={(e) => e.stopPropagation()}
          >
            {photos.map((_, i) => (
              <button
    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <p className="text-xs text-slate-500 dark:text-zinc-400 font-medium mb-4 bg-slate-50 dark:bg-zinc-950 rounded-xl px-3 py-2.5 border border-slate-100 dark:border-zinc-800 truncate flex items-center gap-2">
...
const PriceInsight = ({ price, year, kmDriven }) => {
  ...
  return (
    <div className="flex items-center gap-2.5 shrink-0">
      <span
        className={`inline-flex items-center gap-1.5 text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-full border ${insight.colorClasses}`}
        role="status"
      >
...
const FEATURE_ICON_MAP = {
  sunroof: Wind,
  touchscreen: Tv2,
  androidAuto: MonitorSmartphone,
  appleCarPlay: MonitorSmartphone,
  reverseCamera: ImageIcon,
  parkingSensors: ParkingCircle,
  cruiseControl: Navigation,
  automaticClimateControl: Thermometer,
  leatherSeats: Armchair,
  ...
  const handleBookingRequest = async (requestType) => {
    ...
    try {
      const res = await axios.post(
        ...
        { requestType, note: bookingNote.trim() },
        { headers: authHeaders() },
      );
      if (res.data.success) {
        showToast(
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8">
          ...
          <div className="lg:col-span-7 space-y-5">
                ...
                <div className="absolute bottom-3 left-4 right-4 flex items-center justify-between pointer-events-none">
                  ...
                  {photos.length > 1 && (
                    <span className="bg-black/60 backdrop-blur-sm text-white/80 text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-1.5">
                      <ImageIcon className="w-3 h-3" /> {activePhotoIdx + 1}/
                      {photos.length}
                    </span>
                  )}
                </div>
            ...
            {/* ── DESCRIPTION ── */}
            <div className="bg-white capitalize dark:bg-zinc-900 border border-slate-200/60 dark:border-zinc-800 rounded-3xl p-6 shadow-sm">
              <SectionHeader
                icon={FileText}
                title="Description"
                accent="blue"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 w-full auto-rows-fr">
                ...
                <StatCard
                  ...
                  value={
                    vehicle?.mileage ? (
                      <>
                        {vehicle.mileage}{" "}
                        <span className="uppercase">km/l</span>
                      </>
                    ) : (
                      "N/A"
                    )
                ...
                <StatCard
                  ...
                  value={
                    vehicle?.power ? (
                      <>
                        {vehicle.power} <span className="uppercase">hp</span>
                      </>
                    ) : (
                      "N/A"
                    )
                ...
                <StatCard
                  ...
                  value={
                    vehicle?.torque ? (
                      <>
                        {vehicle.torque} <span className="uppercase">Nm</span>
                      </>
                    ) : (
                      "N/A"
                    )
                  ...
                  <StatCard
                    ...
                    value={
                      vehicle?.topSpeed ? (
                        <>
                          {vehicle.topSpeed}{" "}
                          <span className="uppercase">km/h</span>
                        </>
                      ) : (
                        "N/A"
                      )
            ...
            <CollapsibleCard title="Condition Details" icon={ShieldCheck}>
              <div className="rounded-2xl overflow-hidden border border-slate-100 dark:border-zinc-800">
                {CONDITION_ROWS.map((item, i) => (
                  <div
                    key={i}
                    className={`flex items-center justify-between px-5 py-3.5 text-sm
                      ${i % 2 === 0 ? "bg-slate-50/70 dark:bg-zinc-950/50" : "bg-white dark:bg-zinc-900"}
                      ${i < CONDITION_ROWS.length - 1 ? "border-b border-slate-100 dark:border-zinc-800" : ""}`}
                  >
                    <div className="flex items-center gap-2">
                    ...
                    <span className="font-bold text-slate-850 dark:text-zinc-200">
                      {vehicle[item.key] || item.fallback}
                    </span>
                  </div>
                ))}
              </div>
            ...
            {/* ── FEATURES & COMFORT ── */}
            <CollapsibleCard title="Features & Comfort" icon={ListChecks}>
              <div className="flex flex-wrap gap-2">
                {Object.entries(vehicle.features || {}).map(([key, val]) => {
                  if (!val) return null;
                  const FeatureIcon = FEATURE_ICON_MAP[key] || CheckCircle;
                  return (
                    <span
                      key={key}
                    ...
                    >
                      <FeatureIcon className="w-3.5 h-3.5 text-emerald-500 shrink-0" />
                ...
                {(!vehicle.features ||
                  ...
                  <span className="text-slate-400 text-xs font-medium flex items-center gap-1.5">
                    <Info className="w-3.5 h-3.5" /> No custom features listed
                    for this vehicle.
                  </span>
                )}
              </div>
          ...
          <div className="lg:col-span-5 space-y-5">
            ...
            >
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 dark:border-zinc-800 flex items-end justify-between gap-3">
                <div>
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 dark:text-zinc-500 mb-1 flex items-center gap-1">
                ...
                <div className="text-right">
                  <p className="text-[9px] font-black uppercase tracking-widest text-slate-400 dark:text-zinc-500 mb-1 flex items-center gap-1 justify-end">
                    <Gauge className="w-3 h-3" /> Total KM Driven
                  </p>
            ...
            {/* ── OVERVIEW ── */}
            <CollapsibleCard title="Overview" icon={ClipboardList}>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 w-full auto-rows-fr">
                {[
            ...
            {/* ── SELLER INFORMATION ── */}
            <CollapsibleCard title="Seller Information" icon={User}>
              <div className="space-y-4">
                <div className="flex items-center gap-4 pb-4 border-b border-slate-100 dark:border-zinc-800/60">
                  <div className="w-14 h-14 rounded-2xl uppercase flex items-center justify-center text-white text-2xl font-black shadow-sm shrink-0 bg-linear-to-br from-indigo-500 to-blue-600">
                    {(
                      vehicle.sellerName ||
                ...
                <div className="rounded-3xl bg-slate-50 dark:bg-zinc-950/40 border border-slate-200 dark:border-zinc-800 p-4 space-y-3">
                  {bookingRequest ? (
                    ...
                    <div className="rounded-2xl bg-red-50 dark:bg-red-950/30 border border-red-200 dark:border-red-800 p-4 text-sm font-bold text-red-700 dark:text-red-200 flex items-start gap-2">
                      <AlertCircle className="w-4 h-4 shrink-0 mt-0.5" />
                      This vehicle is sold. Booking and test drive requests are
                      disabled.
                    </div>
                  ) : vehicle.status === "Hidden" ? (
                    <div className="rounded-2xl bg-slate-50 dark:bg-zinc-950/30 border border-slate-200 dark:border-zinc-800 p-4 text-sm font-bold text-slate-600 dark:text-slate-300 flex items-start gap-2">
                      <Info className="w-4 h-4 shrink-0 mt-0.5" />
                      This listing is hidden from public search. You can still
                      contact the garage directly.
                    </div>
                  ) : vehicle.status === "Booked" ? (
                    <div className="rounded-2xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-900 p-4 text-sm font-bold text-amber-700 dark:text-amber-200 flex items-start gap-2">
                    ...
                    <div className="space-y-3">
                      ...
                      <button
                        ...
                        className="w-full py-3 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-black text-sm transition-all shadow-md shadow-emerald-500/20 flex items-center justify-center gap-2"
                      >
                        {bookingSubmitting ? (
                          <>
                ...
                <div className="pt-2.5 space-y-2.5">
                  {whatsappLink ? (
                    ...
                    <div className="text-center py-3 bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-2xl">
                      <p className="text-xs font-bold text-slate-400 dark:text-zinc-500 flex items-center justify-center gap-1.5">
