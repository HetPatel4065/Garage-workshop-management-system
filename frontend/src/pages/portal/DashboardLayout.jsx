...
const tabLinks = [
  ...
  },
  {
    id: "vehicles",
    label: "My Vehicles",
    to: "/portal/dashboard/vehicles",
    icon: FaCar,
  },
  {
    id: "jobcards",
    label: "Job Cards",
    to: "/portal/dashboard/job-cards",
    icon: FileText,
  },
  {
    id: "services",
    label: "Services",
    to: "/portal/dashboard/services",
    icon: Wrench,
  },
  {
    id: "invoices",
    label: "Invoices",
    to: "/portal/dashboard/invoices",
    icon: HiDocumentCurrencyRupee,
  },
  {
    id: "sell",
    label: "Pre-Owned Cars",
    to: "/portal/dashboard/pre-owned-cars",
    icon: Tag,
  },
];

const getTabIdFromPathname = (pathname) => {
  const normalized = pathname.replace(/\/+$/, "");
  if (normalized.endsWith("/vehicles")) return "vehicles";
  if (normalized.endsWith("/job-cards")) return "jobcards";
  if (normalized.endsWith("/services")) return "services";
  if (normalized.endsWith("/invoices")) return "invoices";
  if (normalized.endsWith("/pre-owned-cars")) return "sell";
  return "overview";
};
...
const DashboardLayout = () => {
  ...
  const currentTab = getTabIdFromPathname(location.pathname);

  const jobExpand = useExpandableId(currentTab);
  const svcExpand = useExpandableId(currentTab);
  const vehicleExpand = useExpandableId(currentTab);
  const invoiceExpand = useExpandableId(currentTab);
  ...
  const fetchTabData = useCallback(
    ...
    [vehicles.length, jobCards.length, services.length, fetchInvoicesData],
  );
  ...
  useEffect(() => {
    if (!token) return;
    fetchTabData(currentTab, token, selectedCustomerId);
  }, [currentTab, selectedCustomerId, token, fetchTabData]);

  if (initialLoading) {
    return (
      <div className="min-h-screen bg-[#f8faff] dark:bg-zinc-950 flex flex-col items-center justify-center gap-4 p-4 transition-colors duration-300">
        <Loader2 className="w-10 h-10 sm:w-12 sm:h-12 text-blue-600 dark:text-blue-500 animate-spin" />
    ...
    <div className="min-h-screen bg-[#f8faff] dark:bg-zinc-950 dark:text-zinc-100 font-sans selection:bg-blue-100 dark:selection:bg-blue-900/50 selection:text-blue-600 dark:selection:text-blue-400 flex flex-col transition-colors duration-300">
      ...
      <div className="sticky top-0 z-50 shrink-0 w-full bg-white dark:bg-zinc-900 shadow-xs border-b border-slate-200/60 dark:border-zinc-800">
          ...
          <div className="max-w-7xl mx-auto flex items-center justify-between gap-4 min-w-0">
            ...
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                ...
                <span className="text-[10px] sm:text-xs font-medium tracking-normal text-blue-600 dark:text-blue-400">
                  Customer Portal
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 sm:gap-4 shrink-0">
              <div className="hidden md:flex flex-col items-end min-w-0 max-w-48">
                <p className="text-sm font-bold text-slate-900 dark:text-zinc-200 truncate w-full text-right">
                  {user?.name || "Customer"}
                </p>
                <p className="text-xs font-medium text-slate-500 dark:text-zinc-400 truncate w-full text-right">
                  {user?.email}
                </p>
              </div>
        ...
        {isStaffPortalSession && (
          <div className="w-full bg-linear-to-r from-indigo-100 via-indigo-100/80 to-indigo-200/50 dark:from-indigo-950/40 dark:via-indigo-950/60 dark:to-indigo-900/20 border-b border-amber-400/80 dark:border-amber-900/80 px-4 sm:px-6 py-2.5 transition-all duration-300 shadow-sm backdrop-blur-sm">
...
/* ─────────────────────────────────────────────
   Overview Tab
───────────────────────────────────────────── */
const PortalDashboardOverview = () => {
  const { overviewData, getStatusColor } = useOutletContext();
  const navigate = useNavigate();
  const {
    ...
    >
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-100 dark:border-zinc-800 shadow-xs overflow-hidden transition-colors duration-300">
        ...
        <div className="p-4 sm:p-6">
          {overviewVehicles &&
          overviewVehicles.filter((v) => v.nextServiceDate).length > 0 ? (
              ...
              {overviewVehicles
                ...
                .map((vehicle) => {
                  ...
                  return (
                    <div
                      key={vehicle._id}
                      className={`flex flex-col justify-between gap-4 p-4 rounded-2xl border bg-white dark:bg-zinc-900/50 hover:shadow-sm transition-all duration-300 ${containerBorder}`}
                    >
                      {/* Vehicle identity */}
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          <h4 className="text-sm sm:text-base font-black text-slate-800 dark:text-zinc-200 truncate leading-tight">
                            {vehicle.make} {vehicle.model}
                          </h4>
                          <span className="inline-block mt-1 text-[9px] font-black uppercase bg-slate-600 dark:bg-zinc-700 text-white px-1.5 py-0.5 rounded-sm tracking-wider">
            ...
            <div className="flex flex-col items-center justify-center py-10 gap-2 text-center">
              <Clock className="w-10 h-10 text-slate-300 dark:text-zinc-700" />
      ...
      <div className="bg-white dark:bg-zinc-900 rounded-3xl border border-slate-100 dark:border-zinc-800 shadow-xs overflow-hidden transition-colors duration-300">
        <div className="p-4 sm:p-6 border-b border-slate-100 dark:border-zinc-800 flex items-center justify-between gap-4">
          ...
          <button
            onClick={() => navigate("/portal/dashboard/services")}
            className="text-xs sm:text-sm text-blue-600 dark:text-blue-400 font-bold hover:underline shrink-0"
          >
            View All
          </button>
        </div>
        <div className="p-4 sm:p-6">
          {overviewServices.length > 0 ? (
            ...
            <p className="text-slate-400 dark:text-zinc-500 text-center py-8 text-sm">
...
/* ─────────────────────────────────────────────
   Vehicles Tab
───────────────────────────────────────────── */
const PortalDashboardVehicles = () => {
  const { vehicles, vehicleExpand } = useOutletContext();
      ...
      <div className="p-4 sm:p-6 space-y-3">
        {vehicles.length > 0 ? (
          ...
          <div className="py-12 flex flex-col items-center gap-2 text-center">
            <FaCar className="w-10 h-10 text-slate-300 dark:text-zinc-700" />
...
/* ─────────────────────────────────────────────
   Job Cards Tab
───────────────────────────────────────────── */
const PortalDashboardJobCards = () => {
  const { jobCards, jobExpand } = useOutletContext();
    ...
    >
      ...
      <div className="p-4 sm:p-6 space-y-3">
        {jobCards.length > 0 ? (
          ...
          <div className="py-12 flex flex-col items-center gap-2 text-center">
            <FileText className="w-10 h-10 text-slate-300 dark:text-zinc-700" />
            <p className="text-slate-400 dark:text-zinc-500 text-sm font-medium">
...
/* ─────────────────────────────────────────────
   Services Tab
───────────────────────────────────────────── */
const PortalDashboardServices = () => {
  const { services, svcExpand } = useOutletContext();
    ...
    >
      ...
      <div className="p-4 sm:p-6 space-y-3">
        {services.length > 0 ? (
          ...
          <div className="py-12 flex flex-col items-center gap-2 text-center">
            <Wrench className="w-10 h-10 text-slate-300 dark:text-zinc-700" />
            <p className="text-slate-400 dark:text-zinc-500 text-sm font-medium">
              No logs recorded.
            </p>
          </div>
        ...
        {invoices.length > 0 ? (
          invoices.map((invoice) => (
            <InvoiceRow
              ...
              getStatusColor={getStatusColor}
              onRefresh={() => fetchInvoicesData(token, selectedCustomerId)}
              token={token}
          ...
          <div className="py-12 flex flex-col items-center gap-2 text-center">
            <HiDocumentCurrencyRupee className="w-10 h-10 text-slate-300 dark:text-zinc-700" />
            <p className="text-slate-400 dark:text-zinc-500 text-sm font-medium">
...
/* ─────────────────────────────────────────────
   Pre-Owned Cars Tab
───────────────────────────────────────────── */
const PortalDashboardPreOwnedCars = () => {
  const { token, user, isStaffPortalSession, selectedCustomerId } =
    useOutletContext();
